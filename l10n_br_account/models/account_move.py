...
class AccountMove(models.Model):
    ...
    @api.depends(
        # "line_ids.matched_debit_ids.debit_move_id.move_id.payment_id.is_matched",  # payment_id removed in v18
        "line_ids.matched_debit_ids.debit_move_id.move_id.line_ids.amount_residual",
        "line_ids.matched_debit_ids.debit_move_id.move_id.line_ids.amount_residual_currency",
        # "line_ids.matched_credit_ids.credit_move_id.move_id.payment_id.is_matched",  # payment_id removed in v18
        ...
        # "line_ids.payment_id.state",  # payment_id removed in v18
    ...
    def _compute_amount(self):
        ...
        for move in self.filtered(lambda m: m.fiscal_operation_id):
            ...
            inv_line_ids = move.line_ids.filtered(
                ...
                and line.fiscal_document_line_id  # Only lines with fiscal document
            )
            # Use safe field access - skip if no fiscal lines
            if inv_line_ids:
                ...
                move.amount_tax_signed = sign * sum(
                    inv_line_ids.mapped("amount_tax")
                )
                move.amount_total = sum(inv_line_ids.mapped("amount_total"))
    ...
    def _get_protected_vals(self, vals, records):
        ...
        """
        protected = set()
        # Use sudo() to avoid permission errors when checking fiscal_document_id/line_id
        records_sudo = records.sudo()
        for fname in vals:
            if (
                records._name == "account.move"
                and records_sudo.fiscal_document_id
                and records_sudo.fiscal_document_id._fields.get(fname)
            ):
                continue
            elif (
                records._name == "account.move.line"
                and records_sudo.fiscal_document_line_id
                and records_sudo.fiscal_document_line_id._fields.get(fname)
            ):
                continue
    ...
    def open_fiscal_document(self):
        ...
        Open the list view in the case of several fiscal documents.
        """
        self.ensure_one()
