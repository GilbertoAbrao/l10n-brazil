...
class AccountMoveLine(models.Model):
    ...
    def _compute_name(self):
        ...
        """
        # Use sudo() to access move_id without permission issues
        payment_term_lines = self.filtered(
            lambda line: line.display_type == "payment_term"
            and line.document_type_id
            and line.sudo().move_id.document_number
            and line.payment_term_number
        )
        for line in payment_term_lines:
            # set label for payment term lines. Ex: '0001/1-3'
            line.name = f"{line.sudo().move_id.document_number}/{line.payment_term_number}"

        other_lines = self - payment_term_lines
        if other_lines:
            return super(AccountMoveLine, other_lines)._compute_name()
        return True
    ...
    def create(self, vals_list):
        for values in vals_list:
            ...
            try:
                # Use sudo() to bypass permission checks when accessing parent move
                move_id = self.env["account.move"].sudo().browse(values["move_id"])
                if not move_id.exists():
                    continue
                ...
                # If we can't access the move due to permissions, log and skip
                _logger.warning(
                    "AccessError when accessing account.move %s in create: %s",
                    values.get("move_id"),
                    e,
                )
                continue
            except Exception as e:
                # Other errors, log and skip fiscal document creation
                _logger.exception(
                    "Error accessing account.move %s in create", values.get("move_id")
                )
                continue
        ...
        # Remove discount_percentage field if present (deprecated in Odoo 18)
        # Also ensure fiscal_operation_line_id is set when fiscal_operation_id exists
        for vals in vals_list:
            vals.pop("discount_percentage", None)

            # If fiscal_operation_id is set but fiscal_operation_line_id is not,
            # we need to determine it (onchange doesn't run during create)
            if vals.get("fiscal_operation_id") and not vals.get("fiscal_operation_line_id"):
                try:
                    fiscal_op = self.env["l10n_br_fiscal.operation"].sudo().browse(
                        vals["fiscal_operation_id"]
                    )
                    if fiscal_op.exists():
                        # Get necessary data for line_definition
                        ...
                        if company_id and product_id:
                            ...
                            fiscal_op_line = fiscal_op.line_definition(
                                company=company,
                                partner=partner,
                                product=product,
                            )
                    ...
                    _logger.warning(
                        "Error determining fiscal_operation_line_id: %s", e
                    )
    ...
    def _sync_invoice(self, container):
        ...
        def existing():
            return {
                line: {
                    ...
                    "move_type": line.sudo().move_id.move_type,
                }
                for line in container["records"]
                .with_context(
                    skip_invoice_line_sync=True,
                )
                .filtered(lambda line: line.sudo().move_id.is_invoice(True))
            }
        ...
        for line in after:
            if (
                ...
                line_sudo = line.sudo()
                if not line_sudo.move_id.fiscal_operation_id:
                    ...
                    if line.cfop_id and not line.cfop_id.finance_move:
                        ...
                        if not line_sudo.move_id.fiscal_operation_id.deductible_taxes:
                        ...
                        if line_sudo.move_id.fiscal_operation_id.deductible_taxes:
                            unsigned_amount_currency = (
                ...
                amount_currency = unsigned_amount_currency * line_sudo.move_id.direction_sign
    ...
    def _compute_totals(self):
        ...
        # Use sudo() to access move_id without permission issues
        if not self.sudo().move_id.fiscal_operation_id:
            return result
    ...
    def _compute_all_tax(self):
        ...
        for line in self:
            line_sudo = line.sudo()
            sign = line_sudo.move_id.direction_sign
            if line.display_type == "tax":
                line.compute_all_tax = {}
                line.compute_all_tax_dirty = False
                continue
            if line.display_type == "product" and line_sudo.move_id.is_invoice(True):
            ...
            compute_all_currency = line.tax_ids.compute_all(
                ...
                quantity=quantity,
                product=line.product_id,
                partner=line_sudo.move_id.partner_id or line.partner_id,
                is_refund=line.is_refund,
                handle_price_include=handle_price_include,
                include_caba_tags=line_sudo.move_id.always_tax_exigible,
                ...
                frozendict(
                    {
                        ...
                        "partner_id": line_sudo.move_id.partner_id.id or line.partner_id.id,
                        "move_id": line_sudo.move_id.id,
    ...
    def _get_computed_taxes(self):
        ...
        user_type = None
        move_sudo = self.sudo().move_id
        if move_sudo.is_sale_document(include_receipts=True):
            user_type = "sale"
        elif move_sudo.is_purchase_document(include_receipts=True):
            user_type = "purchase"
